CREATE TABLE IF NOT EXISTS order_complete_task (
    task_id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '订单完成补发任务主键',
    ...
    UNIQUE KEY uk_order_complete_task_event_key (event_key),
    KEY idx_order_complete_task_status_retry_time (task_status, next_retry_at)
) COMMENT='订单完成事件补发任务表';
