...
CREATE TABLE IF NOT EXISTS order_result_task (
    task_id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '下单结果补偿任务主键',
    event_key VARCHAR(64) NOT NULL COMMENT '下单结果事件幂等键',
    event_type VARCHAR(64) NOT NULL COMMENT '下单结果事件类型',
    business_key VARCHAR(64) NOT NULL COMMENT '业务主键，当前默认使用 reservationId',
    payload_json JSON NOT NULL COMMENT '下单结果事件载荷',
    ...
    UNIQUE KEY uk_order_result_task_event_key (event_key),
    KEY idx_order_result_task_status_retry_at (task_status, next_retry_at)
) COMMENT='下单结果补偿任务表';
