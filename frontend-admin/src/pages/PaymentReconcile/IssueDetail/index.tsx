...
const PaymentReconcileIssueDetailPage = () => {
  ...
  /**
   * 加载异常详情。
   * 详情页是人工治理前的事实确认页，必须确保异常事实和最近一次处理轨迹来自同一份真实返回。
   */
  const loadIssueDetail = async () => {
    if (!issueId) {
      setData(undefined);
      setErrorMessage('缺少异常标识，无法查询异常详情');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setData(undefined);
      setErrorMessage(
        error instanceof Error ? error.message : '异常详情加载失败，请稍后重试',
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      {loading ? (
        <PageState type="loading" description="异常详情加载中" />
      ) : null}
      {!loading && errorMessage ? (
        <PageState
          ...
          actionText="重新加载"
          onAction={() => void loadIssueDetail()}
        />
      ) : null}
      {!loading && !errorMessage && !data ? (
        <PageState
          type="empty"
          description="没有查询到该异常详情，可能异常记录不存在或已被清理。"
          actionText="返回异常列表"
          onAction={() => history.push('/payment-reconcile/issues')}
        />
      ) : null}
      {!loading && !errorMessage && data ? (
        <>
